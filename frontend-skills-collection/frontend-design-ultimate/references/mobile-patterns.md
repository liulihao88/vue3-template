...
function MobileSelector({ categories }) {
  ...
  return (
    <div className="selector">
      {categories.map((cat) => (
        <div key={cat.name} className={cn('category', expanded === cat.name && 'expanded')}>
          <button className="category-header" onClick={() => setExpanded(expanded === cat.name ? null : cat.name)}>
            <span>{cat.name}</span>
            <ChevronDown className={cn('transition-transform', expanded === cat.name && 'rotate-180')} />
          </button>
