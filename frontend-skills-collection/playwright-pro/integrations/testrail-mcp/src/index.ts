...
if (!config.url || !config.user || !config.apiKey) {
  console.error('Missing TestRail configuration. Set TESTRAIL_URL, TESTRAIL_USER, and TESTRAIL_API_KEY.')
  process.exit(1)
}

const client = new TestRailClient(config)

const server = new Server({ name: 'pw-testrail', version: '1.0.0' }, { capabilities: { tools: {} } })
  ...
  try {
    switch (name) {
      ...
      case 'testrail_add_result': {
        ...
        const result = await client.addResultForCase(args!.run_id as number, args!.case_id as number, resultPayload)
