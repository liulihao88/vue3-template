...
if (!config.username || !config.accessKey) {
  console.error('Missing BrowserStack configuration. Set BROWSERSTACK_USERNAME and BROWSERSTACK_ACCESS_KEY.')
  process.exit(1)
}

const client = new BrowserStackClient(config)

const server = new Server({ name: 'pw-browserstack', version: '1.0.0' }, { capabilities: { tools: {} } })
  ...
  try {
    switch (name) {
      ...
      case 'browserstack_get_builds': {
        const builds = await client.getBuilds((args?.limit as number) ?? 10, args?.status as string | undefined)
        return { content: [{ type: 'text', text: JSON.stringify(builds, null, 2) }] }
      }

      case 'browserstack_get_sessions': {
        const sessions = await client.getSessions(args!.build_id as string, args?.limit as number | undefined)
