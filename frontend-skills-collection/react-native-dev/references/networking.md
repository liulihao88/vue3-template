...
function TaskList({ projectId }: { projectId: string }) {
  ...
  return <FlashList data={tasks} renderItem={({ item }) => <TaskRow task={item} />} estimatedItemSize={56} />
}
```
...
const toggle = useMutation({
  ...
  onMutate: async (task) => {
    ...
    qc.setQueryData<Task[]>(queryKey, (old) => old?.map((t) => (t.id === task.id ? { ...t, done: !t.done } : t)))
    return { prev }
  },
...
export async function requireAuth(req: Request): Promise<string> {
  ...
  if (!header?.startsWith('Bearer ')) throw Response.json({ error: 'unauthorized' }, { status: 401 })
