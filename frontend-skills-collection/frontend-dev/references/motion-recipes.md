  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      setDisplay(
        text
          .split('')
          .map((char, i) => (i < iteration.current ? char : chars[Math.floor(Math.random() * chars.length)]))
          .join(''),
      )
...
export function MorphCard({ id, preview, detail }: { id: string; preview: React.ReactNode; detail: React.ReactNode }) {
